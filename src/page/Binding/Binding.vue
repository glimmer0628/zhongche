<template>
  <div class="bindingPage">
    ...
    <ul class="bindingUl" v-if="binded">
      ...
      <li class="bindingLi">
        <input-item :text="nickname" :title="'昵称'"></input-item>
      </li>
    </ul>
...
<script>
  ...
  export default {
        ...
        if (this.binded) {
          ...
          if (phoneReg.test(this.phoneNum)) {
            if (verifyReg.test(this.verify)) {
              ...
              this.$http.post(this.prefix + 'user/wechat/reqbind', options)
              .then(res => {
                this.loading = false;
    ...
    created () {
      ...
      this.$http.post(this.prefix + 'User/Wechat/getbinding', {openid: this.openid})
      .then(res => {
        if (!res.body.resData.binded) {
          return;
        }
